<template>
  <div v-if="paper" class="max-w-4xl mx-auto px-6 py-16">
    <router-link to="/publications"
      class="text-sm font-bold text-indigo-600 hover:text-indigo-800 transition-colors mb-8 inline-block">
    ...
    <div class="grid md:grid-cols-3 gap-12">
      <div class="md:col-span-2 space-y-12">
        ...
        <section v-if="paper.colab">
          <h2 class="text-xs font-black uppercase tracking-widest text-slate-400 mb-6">Computational Reproducibility
          </h2>
          <div class="space-y-4">
            <template v-if="Array.isArray(paper.colab)">
              <div v-for="(link, index) in paper.colab" :key="index"
                ...
                <a :href="link" target="_blank"
                  class="px-4 py-2 bg-orange-500 text-white text-xs font-bold rounded-lg hover:bg-orange-600 transition">
                  OPEN IN COLAB
                </a>
              </div>
            </template>
            <template v-else>
              <div class="flex items-center justify-between p-4 border border-orange-100 bg-orange-50/30 rounded-xl">
                ...
                <a :href="paper.colab" target="_blank"
                  class="px-4 py-2 bg-orange-500 text-white text-xs font-bold rounded-lg hover:bg-orange-600 transition">
                  OPEN IN COLAB
                </a>
              </div>
