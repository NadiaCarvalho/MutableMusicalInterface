    ...
    <section v-if="item.study_case" class="space-y-16">
      ...
      <div v-if="item.study_case.examples?.length" class="space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div v-for="(ex, index) in item.study_case.examples" :key="index" :id="'example-' + ex.id"
            class="flex flex-col p-6 bg-slate-50 rounded-3xl border border-slate-100">
            <div class="mb-4">
              <!--<span
                class="text-[10px] font-bold px-2 py-1 bg-white border border-slate-200 rounded text-slate-400 uppercase mb-2 inline-block">
                {{ ex.type }}
              </span>-->
            ...
            <div v-if="ex.type === 'video&image'" class="space-y-4 mt-auto">
              <div class="overflow-hidden rounded-xl border border-slate-200 bg-white">
                <img :src="ex.url_image" :alt="ex.name" class="w-full h-auto object-contain max-h-64" />
              </div>
              <div class="overflow-hidden rounded-xl bg-black aspect-video">
                <iframe v-if="ex.url_video?.includes('youtube') || ex.url_video?.includes('vimeo')" :src="ex.url_video"
                  class="w-full h-full" frameborder="0" allowfullscreen>
                </iframe>
                <video v-else controls :src="ex.url_video" class="w-full h-full object-cover"></video>
              </div>
            </div>

            <div v-if="ex.type === 'video'" class="mt-auto overflow-hidden rounded-xl bg-black aspect-video">
              <iframe v-if="ex.url.includes('youtube') || ex.url.includes('vimeo')" :src="ex.url" class="w-full h-full"
